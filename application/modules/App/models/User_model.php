...
//        echo $this->db->last_query();
    ...
    public function getLevelUser($user_id){
        $this->db->select('count(id) as team,user_id,level');
...
// $this->User_model->get_single_record(SELECT sum(tbl_users.package_amount) as business FROM tbl_users INNER JOIN tbl_sponser_count ON tbl_users.user_id = tbl_sponser_count.downline_id WHERE tbl_sponser_count.user_id = 'G11111';)
    public function teamBusiness(){
        $this->db->select('ifnull(sum(tbl_users.package_amount),0) as business, tbl_users.paid_status');
        $this->db->from('tbl_users');
        $this->db->join('tbl_sponser_count', 'tbl_users.user_id = tbl_sponser_count.downline_id');
        $this->db->where(array('tbl_sponser_count.user_id' => $this->session->userdata['user_id']));
        $query = $this->db->get();
        $res = $query->row_array();
        return $res;
    }
