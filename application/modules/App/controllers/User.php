...
class User extends CI_Controller {
    ...
    public function index() {
        if (is_logged_in()) {
            ...
            $response['totalTeam'] = $this->User_model->get_single_record('tbl_sponser_count', 'user_id = "'.$this->session->userdata['user_id'].'"', 'ifnull(count(id),0) as totalTeam');
            // pr($response['totalTeam']);
            ...
            /*incomes */
            // $response['total_income1'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = "'.$this->session->userdata['user_id'].'" and amount > 0 and created_at >= "'.$response['user']['topup_date'].'"', 'ifnull(sum(amount),0) as total_income1');
            // pr($response['total_income1'],true);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(!empty($sponser)){
                        ...
                        if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])){
                            if(!empty($_SESSION['verification_otp'])){
                                if($data['otp'] == $_SESSION['verification_otp']){
                                    session_unset('verification_otp');
                                    $secretKey = '6Lf7tSwdAAAAANS3ECG0paCbzcIj8hRhZuZcuU7a';
                                    $verifyResponse = file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret='.$secretKey.'&response='.$_POST['g-recaptcha-response']);
                                     $responseData = json_decode($verifyResponse);
                                    if($responseData){
                                        ...
                                        $id_number = $repuser_id;//$this->getUserIdForRegister();
                                        ...
                                        $EthWalletAdd = $this->binance->createAccount();
                                        $userData['wallet_private'] = '';
                                        $userData['wallet_public'] = '';
                                        $userData['wallet_address'] = '';

                                        if(!empty($EthWalletAdd)){
                                            $userData['wallet_private'] = '0x'.$EthWalletAdd['private'];
                                            $userData['wallet_public'] = '0x'.$EthWalletAdd['public'];
                                            $userData['wallet_address'] = '0x'.$EthWalletAdd['address'];
                                        }

                                        // $userData['country_code'] = $this->input->post('country');
                                        $userData['email'] = $this->input->post('email');
                                        // $userData['country'] = $this->input->post('country');
                                        $userData['master_key'] = $this->input->post('master_key');
                                        ...
                                        if ($res) {
                                            if(empty($emailCheck['email'])){
                                                ...
                                                $this->User_model->add('tbl_refferal_wallet',$creditReferalZil);
                                            }
                                            if ($userData['position'] == 'R') {
                                                $this->User_model->update('tbl_users', array('last_right' => $userData['upline_id']), array('last_right' => $userData['user_id']));
                                                $this->User_model->update('tbl_users', array('user_id' => $userData['upline_id']), array('right_node' => $userData['user_id']));
                                ...
                                $this->session->set_flashdata('error', 'OTP Expire!');
                                $this->load->view('register', $response);

                            }
                        }else{
                            $this->session->set_flashdata('error', "Please check on the reCAPTCHA box.");
                            $response['message'] = 'Please check on the reCAPTCHA box.';
                             $this->load->view('register', $response);
                        }
                    }else{
                    ...
                    $this->session->set_flashdata('error', "This User ID Already Exist");
                    $this->load->view('register', $response);
                }
                }else{
...
//            pr($response);
    ...
    public function portfolio(){
        if (is_logged_in()) {
            $response['user'] = $this->User_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['user_id']],'*');
            if($response['user']['package_amount'] > 0){
                $date = date('Y-m-d');
            } else {
                $date = date('Y-m-d H:i:s');
            }
            $response['total_income'] = $this->User_model->get_single_record('tbl_income_wallet',"amount > 0 and user_id = '".$this->session->userdata['user_id']."'",'ifnull(sum(amount),0) as total_income');
            $response['today_income'] = $this->User_model->get_single_record('tbl_income_wallet',['user_id' => $this->session->userdata['user_id'],'amount > '=> 0,'created_at >=' => $date],'ifnull(sum(amount),0) as today_income');
            $response['matching_bonus'] = $this->User_model->get_single_record('tbl_income_wallet',['user_id' => $this->session->userdata['user_id'],'type'=> 'matching_bonus'],'ifnull(sum(amount),0) as matching_bonus');
            ...
            $response['self_income'] = $this->User_model->get_single_record('tbl_income_wallet',['user_id' => $this->session->userdata['user_id'],'type'=> 'self_income'],'ifnull(sum(amount),0) as self_income');
            $response['royalty_income'] = $this->User_model->get_single_record('tbl_income_wallet',['user_id' => $this->session->userdata['user_id'],'type'=> 'royalty_income'],'ifnull(sum(amount),0) as royalty_income');

            $response['leftDirect'] = $this->User_model->get_single_record('tbl_users',['sponser_id' => $this->session->userdata['user_id'],'position'=> 'L'],'ifnull(count(id),0) as leftDirect');
            $response['rightDirect'] = $this->User_model->get_single_record('tbl_users',['sponser_id' => $this->session->userdata['user_id'],'position'=> 'R'],'ifnull(count(id),0) as rightDirect');
                $response['business'] = $this->User_model->teamBusiness();



            $this->load->view('portfolio',$response);
        } else {
            redirect('App/User/login');
        }
    }
    ...
    public function income($type) {
        if (is_logged_in()) {
            ...
            $response['user_incomes'] = $this->User_model->get_records('tbl_income_wallet',"user_id = '".$this->session->userdata['user_id']."' and type = '".$type."' order by created_at desc", 'id,user_id,amount,type,description,created_at');
    ...
    public function income_ledgar() {
        if (is_logged_in()) {
            ...
            $response['total_income'] = $this->User_model->get_single_record('tbl_income_wallet',['user_id' => $this->session->userdata['user_id']], 'ifnull(sum(amount),0) as total_income');
            $response['user_incomes'] = $this->User_model->get_records('tbl_income_wallet', "user_id = '".$this->session->userdata['user_id']."'order by created_at desc", 'id,user_id,amount,type,description,created_at');
            $this->load->view('incomes', $response);
        } else {
            redirect('App/User/login');
        }
    ...
    public function addBalance() {
        if (is_logged_in()) {
            ...
            if ($response['data_retrieve']['ids'] > 0) {
                header('Location: ' . base_url() . 'App/User/payBalance');
            }
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                $data = $this->security->xss_clean($this->input->post());
                if ($data['amount'] >= 2) {
//                    pr($data,true);
    ...
    public function payBalance() {
        if (is_logged_in()) {
            ...
            if ($response1['data_retrieve']['ids'] == 0) {
                header('Location: ' . base_url() . 'App/User/addBalance');
            }
    ...
    public function index1() {
        ...
        public function reffrelWalletHistory(){
            if(is_logged_in()){
                $response['records'] = $this->User_model->get_records('tbl_refferal_wallet',['user_id' => $this->session->userdata['user_id']],'*');
                $response['total_income'] = $this->User_model->get_single_record('tbl_refferal_wallet',['user_id' => $this->session->userdata['user_id']],'ifnull(sum(amount),0) as total_income');

                $response['header'] = 'Refferal Wallet History';
                $this->load->view('reffreal_wallet_history',$response);
            }else{
                redirect('App/User/login');
            }
        }
        ...
        public function setting(){
           ...
           $response['matrix_packages'] = $this->User_model->get_matrix_pacakges();
            ...
            $response['club4'] = $this->User_model->get_single_record('tbl_pool4', array('user_id' => $this->session->userdata['user_id'],'org ' =>1), '*');
            $response['popup'] = $this->User_model->get_single_record1('tbl_user_popup', '*');
                $this->load->view('setting',$response);
            } else {
                redirect('App/User/login');
            }
        }
