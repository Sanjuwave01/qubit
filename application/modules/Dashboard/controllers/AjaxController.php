    ...
    public function adsView(){
        if (is_logged_in()) {
            if($this->input->is_ajax_request()){
                ...
                // $check = $this->User_model->get_single_record('tbl_roi',['user_id' => $this->session->userdata['user_id']],'*');
                // if(!empty($check['user_id'])){
                    $this->User_model->update('tbl_roi',['user_id' => $this->session->userdata['user_id'],'type' => 'roi_income'],['status' => 2]);
                    $response['success'] = true;
                    $response['message'] = 'ROI enabled';
                // } else {
                //     $response['message'] = 'You are not eligible for that';
                // }
                echo json_encode($response);
                exit;
            } else {
