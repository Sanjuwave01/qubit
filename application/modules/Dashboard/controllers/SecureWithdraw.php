...
class secureWithdraw extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->api_token = '625416071062485';
    ...
    {   
        if ($this->input->is_ajax_request()) {
            if ($this->input->server('REQUEST_METHOD') == 'GET') {
                $userinfo = $this->User_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['user_id']],'email,name');
                ...
                $message = 'Dear User, Your OTP is '.$this->session->userdata['verification_otp'].' Never share this OTP with anyone, this OTP expire in five minutes. More Info: '.base_url().' From mlmsig';
                ...
                $user['message'] = 'Dear User, Your OTP is '.$this->session->userdata['verification_otp'].' Never share this OTP with anyone, this OTP expire in five minutes. More Info: '.base_url().' From mlmsig';
                $user['name']  = $userinfo['name'];
                $user['email']  = $userinfo['email'];
                // composeMail($userinfo['email'],'OTP','OTP',$message,$display=false);
                // send_crypto_email($userinfo['email'],'OTP',$message,$display=false);
                sendEmail($user);
    ...
    {     if ($this->input->is_ajax_request()) {
            if ($this->input->server('REQUEST_METHOD') == 'GET') {
                ...
                $this->session->mark_as_temp('verification_otp', 300);
                $message = 'You OTP is '.$this->session->userdata['verification_otp'].' (One Time Password), this otp expire in 2 mintues!';
                // $message = 'Dear '.$userinfo['name'].', Your OTP is '.$this->session->userdata['verification_otp'].' Never share this OTP with anyone, this OTP expire in two minutes. More Info: '.base_url().' From mlmsig';
                ...
                send_crypto_email2($userinfo['email'],'Otp',$message);
                notify($this->session->userdata['user_id'],$message, '1201161518339990262', '1207162142573795782');
                if($message){
                    $response['status'] = 1;
    ...
    public function verifyMailOtp($email)
    {     if ($this->input->is_ajax_request()) {
            if ($this->input->server('REQUEST_METHOD') == 'GET') {
                ...
                send_crypto_email2($email,'Email Verify',$message);
                if($message){
                    $response['status'] = 1;
